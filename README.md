BuyKIT is a modern frontend e-commerce project delivering a refined shopping experience for premium football jerseys. Built with HTML5, CSS3, and ES6, it features responsive layouts, elegant UI design, CSS variable theming, and dynamic content, showcasing strong frontend fundamentals, clean architecture,smooth interactions and professional portfolio.

LIVE LINK- https://mh-hamza-28.github.io/BUYKIT/

Technologies Used:

HTML5, CSS3, JavaScript (ES6+)
Styling: CSS Grid, Flexbox, CSS Variables, Custom Animations
Fonts: Google Fonts (Montserrat, Open Sans
Icons: Custom PNG icons
Storage: localStorage for cart persistence

Technical Features:

Modular Architecture - Separated HTML, CSS, and JavaScript files
Dynamic Content - JavaScript-driven product rendering
CSS Variables - Consistent theming with custom properties
Performance Optimized - Efficient image handling and code structure
Cross-browser Compatibility - Tested on modern browsers

Design & UI/UX:

Fully responsive across all device sizes
Color palette with football-themed styling
Smooth animations, hover effects, and transitions
Popup notifications for cart actions(feedback)
Semantic HTML and keyboard navigation support.
